...
public class ListDemoModel {
	...
	private IObjectLoader<IAccumulator> accLoader = new ObjectLoaderPath<IAccumulator>(
		(attemptedClassname, params)->{
			logger.log(LogLevel.ERROR, "Unable to load class: "+attemptedClassname+"  Returning error accumulator.");
			return new IAccumulator() {

				@Override
				public void accumulate(Object x) {
					logger.log(LogLevel.ERROR, "["+attemptedClassname+"] Error accumulator: x = "+x);
				}
				
				@Override
				public String toString() {
					return "["+attemptedClassname+"] Error Accumulator";
				}
	...
	private IObjectLoader<IListAlgo> visLoader = new ObjectLoaderPath<IListAlgo>(
		(attemptedClassname, params)->{
			logger.log(LogLevel.ERROR, "Unable to load class: "+attemptedClassname+"  Returning error visitor.");
			return new IListAlgo() {

				@Override
				public Object emptyCase(MTList host, Object... inp) {
					return "["+attemptedClassname+"] Error algo: Empty case";
				}

				@Override
				public Object nonEmptyCase(NEList host, Object... inp) {
					return "["+attemptedClassname+"] Error algo: Non-empty case";
				}
